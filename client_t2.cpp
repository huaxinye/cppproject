...
using namespace std;
void filetransimisson(int client)
{
    ...
    char bufferfile[bufsize];
    bzero(bufferfile,bufsize);
    strncpy(bufferfile, file_name, strlen(file_name)>bufsize?bufsize:strlen(file_name));
    //向服务器发送buffer中的数据
    send(client,bufferfile,bufsize,0);
...
//    if( fp < 0 )
    ...
    bzero(bufferfile,bufsize);
    int length = 0;
    while( 0!= (length = recv(client,bufferfile,bufsize,0)))
    {
...
void Sendm(char buffers[bufsize],int client,int server,time_t lt,char name[32])
{
    ...
    cout<<name<<": "<<buffers<<"  --message sent by ";
    ptime(lt);
    cout<<'\n';}
    if(*buffers == '*')
    {
    ...
    if(*buffers == '#')
    {filetransimisson(client);}
    }
}


void Recem(char bufferr[bufsize],int client,int server,time_t lt,int length,char name[16])
{
    while(1){
